...
describe("Test de RUTAS", () => {
  describe("GET /rickandmorty/character/:id", () => {
    ...
    it("Si hay un error responde con status: 500", async () => {
      const response = await agent.get("/rickandmorty/character/3958j");
  ...
  describe("DELETE /rickandmorty/fav/:id", () => {
    it("Si el ID solicitado no existe, deberia retornar un arreglo con todos los favoritos", async () => {
      const response = await agent.delete("/rickandmorty/fav/3958jj");
      expect(response.body.length).toBe(2);
    });
